# -*- coding: utf-8 -*-
"""Vivienne_12_15_shared_4580_demo_model.ipynb
...
#Method that ouputs a CSV file containing throughput, ttft, and tbt values for different lambda inputs for each scheduling policy
def policy_testing_results(lambda_values, Q=25, n_queries=1000, seed=1):
    """
    Run all three schedulers for a range of lambda values and return a table
    of throughput per policy at each lambda.

    Parameters
    ----------
    lambda_values : list of float
        List of lambda values (average query arrival rate) to simulate.

    Q : int
        Max queries per batch for the hybrid scheduler.
    ...
    -------
    pandas.DataFrame
        Table with columns: 'Lambda', 'Baseline', 'PrefillFirst', 'Hybrid'
    ...
    for lam in lambda_values:
        # Initialize policies
        baseline_policy = ServeToCompletionPolicy()
        prefill_policy = PrefillFirstPolicy()
        hybrid_policy = HybridSchedulerPolicy(Q=Q)

        # Run simulations
        r_baseline = simulate(baseline_policy, lam=lam, n_queries=n_queries, seed=seed)
        r_prefill = simulate(prefill_policy, lam=lam, n_queries=n_queries, seed=seed)
        r_hybrid  = simulate(hybrid_policy, lam=lam, n_queries=n_queries, seed=seed)

        # Record throughput
        rows1.append({
            'Lambda': lam,
            'Baseline throughput': r_baseline['throughput'],
            'PrefillFirst throughput': r_prefill['throughput'],
            'Hybrid throughput': r_hybrid['throughput']
        })

        # Record ttft
        rows2.append({
            'Lambda': lam,
            'Baseline ttft': r_baseline['ttft'].mean(),
            'PrefillFirst ttft': r_prefill['ttft'].mean(),
            'Hybrid ttft': r_hybrid['ttft'].mean()
        })

        #record tbt
        rows3.append({
            'Lambda': lam,
            'Baseline tbt': r_baseline['tbt'].mean(),
            'PrefillFirst tbt': r_prefill['tbt'].mean(),
            'Hybrid tbt': r_hybrid['tbt'].mean()
        })

    throughput = pd.DataFrame(rows1)
    ttft = pd.DataFrame(rows2)
    tbt = pd.DataFrame(rows3)

    results = pd.merge(throughput, ttft, on='Lambda')
    results = pd.merge(results, tbt, on='Lambda')

    return results


# Run sweep
results_df = policy_testing_results(
    [0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6, 6.5, 7, 7.5, 8, 8.5, 9, 9.5, 10],
    Q=15, n_queries = 3000)

# Save to CSV
results_df.to_csv("policy_testing_results_3000.csv", index=False)

print("Results saved to policy_testing_results.csv")
...
Seems stable intially but once it hit 0.5-1.0 it seems as if theres a phase transtition once it gets overloaded.

For now this makes sense as stable when lower and unstable once lamnda is high but still fine-tuning and validating everything.

CHANGES MADE
-there was an error in how TBT was being stored, numbers were being incorrectly added at the wrong point in the loop of finish batch processing
-the moving average charts were changed into histograms to better take away noisiness

ERRORS
-error in the actual TBT calculation
-was taking time between tokens including from separate queries. That is why when lambda made very small, TBT goes high because of the large time in between tokens of different queries that take a while to come through.
"""
